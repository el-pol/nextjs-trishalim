                        ...
                        <div className='flex space-x-3'>
                            {images.map((image, index) => (
                                <button
                                    ...
                                    className={`h-12 w-12 border rounded-md hover:opacity-100 transition-opacity ${currentSlide === index ? '' : 'opacity-75'}`}
                                    key={index}
                                    onClick={() => setStoreState({ currentSlide: index })}
                                >
                                    <Image src={images[index]} alt={`image slide ${index}`} height='48' width='48' objectFit='cover' />
                                </button>
                            ))}
                        </div>
                    )
